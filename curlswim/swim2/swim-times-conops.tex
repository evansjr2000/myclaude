...
\texttt{usaswimming.sisense.com}.  An optional local Postgres
...
\begin{description}[style=nextline]
  \item[Swimmer selection (\code{-o} flag).]
        ...
        \code{ledecky10}) to restrict output.  When no swimmer
        keyword is given all five swimmers are processed.
        \code{ledecky10} retrieves Katie Ledecky's swims as a
        9- and 10-year-old (17~March~2006--16~March~2008).
  ...
  \item[Database persistence (\code{store} keyword).]
        Each retrieved row is also streamed into the local
        Postgres database \texttt{swim-times} via a child
        \texttt{psql} process and a single \texttt{COPY \dots\
        FROM~STDIN} transaction.  Combine with \code{csv} to also
        echo the rows to standard output.
  ...
  \item[Live authoritative data.]
        Online runs fetch from the USA~Swimming Sisense ElastiCube
        via authenticated JAQL API calls; no local cache is
        consulted unless \code{offline} is selected.
...
\begin{itemize}
  ...
  \item \texttt{psql} on \texttt{\$PATH} (or the
        \texttt{SWIM\_TIMES\_PSQL} environment variable pointing at
        an equivalent client) when the \code{store} or \code{offline}
        keywords are used.  The local Postgres container is started
        by \texttt{pg.sh start} and the \texttt{swim-times} database
        is created by applying \texttt{schema.sql}.
\end{itemize}
...
\begin{description}[style=nextline]
  ...
  \item[Database.]
        Optional local Postgres reached via a child \texttt{psql}
        process: \texttt{COPY swim\_times \dots\ FROM~STDIN} for
        \code{store}, \texttt{SELECT \dots\ FROM swim\_times} for
        \code{offline}.  By project convention the host is not
        assumed to have a \texttt{psql} binary on \texttt{\$PATH};
        all access goes through the \texttt{pg.sh} wrapper, which
        runs \texttt{psql} inside the Postgres container via
        \texttt{docker exec}.  Connection parameters come from
        \texttt{SWIM\_TIMES\_PSQL} (default
        \texttt{../../pg/pg.sh psql}).
...
\begin{enumerate}
  \item Parent runs:
        ...
        \end{center}
  \item swim-times resolves Katie Ledecky's PersonKey, fetches
        all 100~FR~SCY times, filters them to her ninth-through-
        tenth-birthday window, and prints the fastest.
...
other capability.  The binary must be recompiled if the Sisense
bearer token rotates or if USA~Swimming changes its JAQL endpoint
URLs.  These are maintenance events, not operational events.
