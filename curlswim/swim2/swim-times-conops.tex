...
times REST service, retrieves the swimmer's best recorded times, sorts
...
\begin{enumerate}[label={[\arabic*]}]
  ...
  \item USA~Swimming, \textit{Times API (OpenAPI description)},
...
\begin{enumerate}
  \item Live data is fetched directly from the authoritative
        USA~Swimming times REST service, removing the human
...
\begin{description}[style=nextline]
  ...
  \item[Live authoritative data.]
        Online runs fetch from the USA~Swimming times REST service
        via anonymous API calls; no local cache is
...
\begin{itemize}
  ...
  \item \code{libcurl} with TLS support (for HTTPS to the times service).
...
\begin{description}[style=nextline]
  ...
  \item[Network.]
        HTTPS calls to
        \texttt{https://times-api.usaswimming.org/swims/TimesSearch}
        authenticated by anonymous request headers (no embedded
        bearer token).  Skipped under \code{offline}.
...
other capability.  The binary must be recompiled if USA~Swimming
changes its times-service endpoint URLs or authentication scheme.
These are maintenance events, not operational events.

\subsection{Security}

The program embeds no credential.  The public times service
authenticates anonymous callers with three request headers
(\code{Usas-Sub-Id: Anonymous}, \code{AppName: DataHub}, and a
run-time-generated \code{Device-Id}), and USA~Swimming's data hub
serves only public competitive records, so all access is
read-only to non-sensitive data.
