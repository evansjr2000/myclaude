...
\begin{itemize}
  ...
  \item Resolves each selected swimmer's USA~Swimming \code{memberId}
        via the public REST member-search endpoint
        (\code{GetMembersForFilters}).
  \item Retrieves each selected swimmer's best recorded time per event
        from the public REST best-times endpoint
        (\code{GetBestTimesForMember}).
...
\begin{description}[style=nextline,leftmargin=2.5cm,labelindent=0pt]
  ...
  \item[Device-Id] Client-generated identifier the times service
                   requires for anonymous requests.
  \item[LCM] Long-Course Meters.
  \item[memberId] USA~Swimming's per-swimmer identifier (a hexadecimal
                  token) returned by the member-search endpoint.
...
\begin{itemize}
  \item The USA~Swimming times REST service at
        \texttt{times-api.usaswimming.org} for all data.
...
\begin{enumerate}
  ...
  \item Member-search HTTPS POST to resolve a swimmer's
        \code{memberId} from a name substring.
  \item Best-times HTTPS GET per swimmer, returning the best time
        in every event at once.
  \item In-memory insertion sort by computed swim time
        (\code{sort\_key}, smallest first).
...
\begin{description}[style=nextline]
  ...
  \item[Maintainers.]
        ...
        and recompile when new swimmers, events, or output modes
        are added.
\end{description}
...
\begin{description}[style=nextline]
  \item[Network.]
        ...
        to \texttt{times-api.usaswimming.org}.
  \item[Authentication.]
        The public times service authenticates anonymous callers
        with three request headers (\code{Usas-Sub-Id: Anonymous},
        \code{AppName: DataHub}, and a client-generated
        \code{Device-Id}); no bearer token is embedded, and the
        \code{Device-Id} is minted at run time.
...
\begin{itemize}
  \item USA~Swimming continues to expose the public REST endpoints
        \code{GetMembersForFilters} and
        \code{GetBestTimesForMember}.
  \item The best-times records retain their named fields
        (\code{strokeAbbreviation}, \code{distance},
        \code{courseCode}, \code{swimTime}).
...
\begin{description}[style=nextline]
  ...
  \item[\req{REQ-F-13}] When any of the \code{ledecky10},
        ...
        \code{memberId} via the same person-search path used for
        the other swimmers.
...
\begin{description}[style=nextline]
  \item[\req{REQ-F-17}] The program shall be designed to fetch times
        ...
        resolve her USA~Swimming \code{memberId}.  ``Evans, Stella~J''
        in the source roster request is the same swimmer as
        \code{stella} and is not duplicated.
  \item[\req{REQ-F-18}] When the \code{list} keyword is supplied, the
        program shall print every roster swimmer's selection id and
        search name to standard output, one per line, and exit with
        status~0 without issuing any network request or opening the
        database.  (This realises requirement~\#5 of
        \texttt{requirements5.txt}.)
\end{description}
...
\begin{description}[style=nextline]
  \item[\req{REQ-F-30}] For each selected swimmer the program
        ...
        to the \code{GetMembersForFilters} member-search endpoint and
        matching the configured lower-cased substring against the
        returned \code{fullName} field.
  \item[\req{REQ-F-31}] For each selected swimmer the program
        shall issue one HTTPS GET to the
        \code{GetBestTimesForMember} endpoint, scoped by the
        resolved \code{memberId}, returning the member's best time
        in every event.
  \item[\req{REQ-F-32}] From each best-times record the program
        shall read the stroke, distance, course, and formatted swim
        time and reassemble the event code (e.g.\ \code{100 FR SCY}).
        The anonymous feed carries no meet or date; those two fields
        shall be left empty.
  \item[\req{REQ-F-33}] The program shall accommodate up to
        \code{MAX\_TIMES} (200) records per fetch.
  \item[\req{REQ-F-34}] The program shall compute the motivational
        standard for each best time locally, since the anonymous feed
        omits it.  Using the swimmer's gender (from the roster) and age
        group (derived from the age returned by the member search), it
        shall query the \code{motivational\_standard} table
        (\S\ref{sec:req-db}) for the fastest standard the time meets and
        render it in the standard column.  When the database is
        unavailable, the gender is unknown, or the age group is not
        tabulated, the column shall be left empty rather than erroneous.
\end{description}
...
\begin{description}[style=nextline]
  \item[\req{REQ-F-40}] The program shall sort the rows for each
        event by computed swim time in seconds
        (\code{sort\_key}, ascending, smallest = fastest) before
        printing.
  ...
  \item[\req{REQ-F-42}] When a swim date is present the program
        shall reformat it from a \code{YYYYMMDD} integer string to
        \code{YYYY-MM-DD}.
\end{description}
...
\begin{description}[style=nextline]
  ...
  \item[\req{REQ-I-11}] The program shall send the anonymous
        authentication headers \code{Usas-Sub-Id: Anonymous},
        \code{AppName: DataHub}, and a run-time-generated
        \code{Device-Id} on every times-service request.
...
\begin{description}[style=nextline]
  \item[\req{REQ-I-20}] All network communication shall use HTTPS
        (TLS) to the host \texttt{times-api.usaswimming.org}.
\end{description}
...
\begin{description}[style=nextline]
  ...
  \item[\req{REQ-P-02}] A single-swimmer single-event query
        ...
        ten seconds under normal service response time.
...
\begin{description}[style=nextline]
  ...
  \item[\req{REQ-D-11}]\label{sec:req-db} The \texttt{schema.sql} file
        shall also define a \texttt{motivational\_standard} table with
        columns \code{gender}, \code{age\_group}, \code{event},
        \code{standard}, and \code{cut\_seconds}, keyed on
        (\code{gender}, \code{age\_group}, \code{event}, \code{standard}).
        It shall be seeded by \texttt{motivational-standards.sql},
        generated from the official USA~Swimming motivational-standards
        PDF by \texttt{tools/parse-motivational.py}.  This table backs
        the local standard computation of \req{REQ-F-34}.
\end{description}
...
\begin{description}[style=nextline]
  \item[\req{REQ-A-01} (Reliability)] The program shall produce
        identical output for identical service responses on
        repeated invocations.
  \item[\req{REQ-A-02} (Maintainability)] All service endpoint
        URLs and request-header values shall be declared as named
