...
\begin{description}[style=nextline]
  ...
  \item[\req{REQ-F-11}] When no swimmer keyword is supplied to
        ...
        the program shall process all five swimmers.
  \item[\req{REQ-F-12}] The set of selectable swimmers shall be
        ...
        and Katie Genevieve Ledecky (\code{ledecky10}).
  \item[\req{REQ-F-13}] When the \code{ledecky10} keyword is
...
\begin{description}[style=nextline]
  \item[\req{REQ-F-70}] When the \code{store} keyword is supplied,
        the program shall stream every emitted row into the local
        Postgres database \texttt{swim-times} via a single
        \texttt{COPY swim\_times \dots\ FROM~STDIN} transaction
        spawned through a child \texttt{psql} process.
  ...
  \item[\req{REQ-F-74}] If \texttt{psql} cannot be spawned for
        \code{store}, the program shall print a single warning to
        standard error and continue with the run, without
        persisting any rows.
\end{description}
...
\begin{description}[style=nextline]
  ...
  \item[\req{REQ-I-11}] The program shall send the headers
        ...
        request.
  \item[\req{REQ-I-12}] The program shall reach Postgres only
        through a child \texttt{psql} process spawned via
        \texttt{popen(3)}.  The shell prefix shall be taken from
        the \texttt{SWIM\_TIMES\_PSQL} environment variable when
        set, otherwise the default \texttt{../../pg/pg.sh psql}
        (the \texttt{pg.sh} wrapper resolved relative to the
        \texttt{swim2/} working directory).  No direct
        \texttt{libpq} dependency shall be linked into the binary.
  \item[\req{REQ-I-13}] The program shall not assume that a host
        \texttt{psql} client is installed on \texttt{\$PATH}.  All
        Postgres access (both \code{store} writes and \code{offline}
        reads) shall go through the \texttt{pg.sh} wrapper, which
...
\begin{description}[style=nextline]
  \item[\req{REQ-P-01}] A full refresh of all five swimmers
        across all thirty-one default events
        ($5 \times (1 + 31) = 160$ HTTP requests) shall complete
        within five minutes on a residential broadband connection.
...
\begin{description}[style=nextline]
  ...
  \item[\req{REQ-D-04}] No third-party dependency other than
        \code{libcurl} and the standard C library shall be linked
        into the binary.
...
\begin{longtable}{ll}
  ...
  REQ-F-10 \ldots REQ-F-14 & Demonstration \\
  ...
  REQ-F-70 \ldots REQ-F-74 & Test (DB-gated) \\
