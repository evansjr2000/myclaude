...
\begin{longtable}{ll}
  ...
  \code{Type definitions}         & \code{Buffer} and \code{TimeRow} structs       \\
...
\begin{tabular}{ll}
  ...
  \code{main}           & \code{lookup\_person\_key}, \code{fetch\_times} \\
...
The dependency graph is acyclic; the only external dependency
is \code{libcurl}.
...
tracked swimmer in the Sisense Persons table.
...
Two POST endpoints are consumed:
\begin{itemize}
...
\begin{enumerate}
  ...
  \item \code{main} initialises libcurl (\code{curl\_global\_init}).
  \item For each entry in \code{SWIMMERS} not masked out by
        \code{g\_opts}:
        \begin{enumerate}
          \item \code{lookup\_person\_key} issues one POST and
                returns a heap-allocated \code{PersonKey} string.
          \item For each requested event (or all default events),
                \code{fetch\_times} issues one POST, parses the
                response into a \code{TimeRow} array, sorts it,
                and prints it.
        \end{enumerate}
  \item \code{main} calls \code{curl\_global\_cleanup} and exits.
\end{enumerate}
