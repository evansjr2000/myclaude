...
\begin{description}[style=nextline]
  ...
  \item[Database persistence sub-process.]
        \req{REQ-F-70} \dots\ \req{REQ-F-73} exercise the
        \code{store} write path and the \code{offline} read path
        against a local Postgres container.  All access goes
        through \texttt{pg.sh psql} (\req{REQ-I-13}), which runs
        \texttt{psql} inside the container via
        \texttt{docker exec}; the host need not have a
        \texttt{psql} client installed.  The companion helper
        \code{test-db.sh} starts the container via \texttt{pg.sh},
        applies \texttt{schema.sql}, and prints a summary of
        \texttt{swim\_times}.  Tests are skipped cleanly when
        \texttt{pg.sh psql} cannot reach the \texttt{swim-times}
        database.
\end{description}
...
is checked in.  No synthetic Sisense data is required because
...
\begin{longtable}{p{2.6cm}p{1.0cm}p{8.4cm}}
  ...
  \req{REQ-F-11}    & D & \code{-e ...} with no swimmer keyword yields five \code{Swimmer:} lines \\
  \req{REQ-F-12}    & D & \code{stella} keyword resolves to a row containing ``Evans'' \\
  \req{REQ-F-13}    & D & \code{ledecky10} resolves to a row containing ``Katie Genevieve Ledecky'' \\
  ...
  \req{REQ-F-74}    & I & Missing-\texttt{psql} warning path inspected in \code{db\_open} \\
  ...
  \req{REQ-I-12}    & I & \code{db\_psql\_command} reads \texttt{SWIM\_TIMES\_PSQL}; binary linkage excludes \texttt{libpq} \\
  \req{REQ-I-13}    & I & \code{db\_psql\_command} default is \texttt{../../pg/pg.sh psql}; no host \texttt{psql} on \texttt{\$PATH} required \\
