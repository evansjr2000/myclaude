...
verification of upstream USA~Swimming data, the back-end REST
services, or the unrelated Pandoc-based PDF report at
\code{swim/swim-times.md}.
...
swim times from the USA~Swimming data hub via its public REST times service.
...
\begin{itemize}
  ...
  \item The public times service accepts anonymous requests
...
\begin{longtable}{p{4.0cm}p{1.5cm}p{1.5cm}p{6cm}}
  ...
  \textbf{Risk} & \textbf{Likeli-} & \textbf{Impact} & \textbf{Mitigation} \\
                & \textbf{hood}     &                  & \\
  \midrule\endhead
  Anonymous auth scheme changes
            & Med  & High & Update auth headers, recompile, retest;
                            trigger STP re-run. \\
  Times-service response schema change
...
\begin{description}[style=nextline]
  ...
  \item[Online behaviour sub-process.]
        ...
        HTTP requests against the times REST service.  Skipped
...
is checked in.  No synthetic API data is required because
...
\begin{itemize}
  ...
  \item More than three consecutive online tests fail with
        identical HTTP error codes (suggests a service-side
...
\begin{tabular}{ll}
  ...
  Anonymous auth scheme changed                 & Run STP after recompile          \\
...
\begin{longtable}{p{2.6cm}p{1.0cm}p{8.4cm}}
  ...
  \req{REQ-F-14}    & T & All \code{ledecky10} dates fall in [2006-03-17, 2008-03-16]  \\
  \req{REQ-F-15}    & T & All \code{ledecky12} dates fall in [2008-03-17, 2010-03-16]  \\
  \req{REQ-F-16}    & T & All \code{ledecky14} dates fall in [2010-03-17, 2012-03-16]  \\
  \req{REQ-F-18}    & T & \code{-o list} prints every roster id to stdout, exits 0, no network \\
  ...
  \req{REQ-I-11}    & I & Anonymous auth header strings (\code{Usas-Sub-Id}, \code{AppName}, \code{Device-Id}) present in source \\
  \req{REQ-I-12}    & I & \code{db\_conn\_string} reads \texttt{SWIM\_TIMES\_PGCONNINFO}; binary linkage includes \texttt{libpq} \\
  \req{REQ-I-20}    & A & \code{https://times-api.usaswimming.org} substring present, no \code{http://} \\
