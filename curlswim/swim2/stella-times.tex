...
This program fetches best short-course yard (SCY) times for four particular
swimmers:
...
\Y\B\X9:Includes\X\6
\X10:Global constants\X\6
\X11:Option flags\X\6
\X13:Type definitions\X\6
\X16:HTTP utilities\X\6
\X20:JSON scanner\X\6
\X23:Person lookup\X\6
\X25:Times fetch\X\6
\X29:Main function\X\par
\fi
...
\M{10}The Sisense bearer token authenticates us against the analytics
instance that backs \.{data.usaswimming.org}.  \PB{\.{EVENTS}} lists the
twelve SCY event codes we query.

\Y\B\4\X10:Global constants\X${}\E{}$\6
...
\M{11}The program accepts two optional flags on the command line.
...
\item{$\bullet$} \.{csv} --- emit CSV lines (swimmer name on every line)
instead of a table.
\medskip\noindent
...
only.  The flag may also be repeated (e.g.\ \.{-e "100 FR SCY" -e "50 FL
SCY"}).
...
\Y\B\4\X11:Option flags\X${}\E{}$\6
\8\#\&{define} \.{OPT\_STELLA}\5${}(\T{1}\LL\T{0}{}$)\C{ restrict output to
Stella's events         }\6
\8\#\&{define} \.{OPT\_KALEA}\5${}(\T{1}\LL\T{1}{}$)\C{ restrict output to
Kalea's events          }\6
\8\#\&{define} \.{OPT\_FASTEST}\5${}(\T{1}\LL\T{2}{}$)\C{ print only the single
fastest time         }\6
\8\#\&{define} \.{OPT\_CSV}\5${}(\T{1}\LL\T{3}{}$)\C{ emit CSV lines instead of
a table          }\6
\8\#\&{define} \.{OPT\_KENNY}\5${}(\T{1}\LL\T{4}{}$)\C{ restrict output to
Kenneth Ray Evans       }\6
\8\#\&{define} \.{OPT\_KEITH}\5${}(\T{1}\LL\T{5}{}$)\C{ restrict output to
Keith Santiago Evans    }\6
\&{static} \&{int} \\{g\_opts}${}\K\T{0}{}$;\C{ output-selection flags; 0 =
show everything }\6
\&{static} \&{char} \\{g\_events}[\.{NUM\_EVENTS}][\T{32}];\C{ event codes
requested via \.{-e}  }\6
\&{static} \&{int} \\{g\_nevents}${}\K\T{0}{}$;\C{ number of entries in \PB{%
\\{g\_events}}             }\par
\U7.\fi
...
\M{13}A \PB{\\{Buffer}} holds a dynamically-grown heap string.  \.{libcurl}
...
\M{14}A \PB{\\{TimeRow}} records one swim result.  \PB{\\{sort\_key}} is a
numeric proxy
...
\Y\B\4\X13:Type definitions\X${}\mathrel+\E{}$\6
\&{typedef} \&{struct} ${}\{{}$\1\6
\&{double} \\{sort\_key};\6
\&{char} \\{time}[\T{32}];\6
\&{char} \\{date}[\T{16}];\6
...
\N{1}{15}HTTP utilities.

\fi

\M{16}The \.{libcurl} write callback appends each incoming chunk to a
...
\Y\B\4\X16:HTTP utilities\X${}\E{}$\6
...
\M{17}\PB{\\{post\_json}} performs a single HTTP~POST with a JSON body and
returns
the response as a null-terminated heap string, or \PB{$\NULL$} on failure.
The caller is responsible for freeing the returned string.

\Y\B\4\X16:HTTP utilities\X${}\mathrel+\E{}$\6
...
\M{19}Sisense JAQL responses contain a top-level \PB{\.{"values"}} array.  Each
...
\M{20}\PB{\\{scan\_string}} finds the first \PB{\.{"key"}} at or after \PB{${*}%
\\{pos}$}, copies the
...
\M{21}\PB{\\{scan\_long}} is the numeric counterpart: it finds \PB{\.{"key"}},
converts
the digits that follow to a \PB{\&{long}}, stores the result in \PB{${*}%
\\{out}$}, and
advances \PB{${*}\\{pos}$}.

\Y\B\4\X20:JSON scanner\X${}\mathrel+\E{}$\6
...
\N{1}{22}Person lookup. \PB{\\{lookup\_person\_key}} posts a search for \PB{%
\\{search\_query}} and scans
...
\M{25}\PB{\\{insertion\_sort}} sorts \PB{\|n} \PB{\&{TimeRow}} records in-place
by \PB{\\{sort\_key}}
...
\M{26}\PB{\\{format\_date}} converts an 8-digit \.{YYYYMMDD} string (such as
\.{20230305}) to \.{YYYY-MM-DD} format in \PB{\\{out}} (which must be at
least 11 bytes).

\Y\B\4\X25:Times fetch\X${}\mathrel+\E{}$\6
...
\M{27}\PB{\\{fetch\_times}} retrieves all SCY times for \PB{\\{person\_key}} in
...
from
\.{YYYYMMDD} to \.{YYYY-MM-DD} by \PB{\\{format\_date}}.  \PB{\\{swimmer%
\_name}} is
the resolved full name of the swimmer (used in CSV output).
When \PB{\\{opts}} has \PB{\.{OPT\_CSV}} set the output is a comma-separated
line per
time record with the swimmer name on every line; when \PB{\.{OPT\_FASTEST}} is
set only the single fastest time is printed.

\Y\B\4\X25:Times fetch\X${}\mathrel+\E{}$\6
\1\1\&{static} \&{void} \\{fetch\_times}(\&{const} \&{char} ${}{*}\\{person%
\_key},\39{}$\&{const} \&{char} ${}{*}\\{event\_code},\39{}$\&{const} \&{char}
${}{*}\\{swimmer\_name},\39{}$\&{int} \\{opts})\2\2\6
${}\{{}$\1\6
\&{char} \\{url}[\T{512}];\7
${}\\{snprintf}(\\{url},\39{}$\&{sizeof} \\{url}${},\39\.{"%
...
\4${}\}{}$\2\6
\\{free}(\\{resp});\6
${}\\{insertion\_sort}(\\{rows},\39\\{nrows});{}$\7
\&{int} \\{lim}${}\K(\\{opts}\AND\.{OPT\_FASTEST})\?(\\{nrows}>\T{0}\?\T{1}:%
\T{0}):\\{nrows};{}$\7
\&{if} ${}(\\{opts}\AND\.{OPT\_CSV}){}$\5
${}\{{}$\C{ CSV: swimmer,event,time,date,standard,meet  (no header here;
    caller emits the header once before the first swimmer loop). }\1\6
\&{for} (\&{int} \|i${}\K\T{0};{}$ ${}\|i<\\{lim};{}$ ${}\|i\PP){}$\1\5
${}\\{printf}(\.{"\\"\%s\\",\\"\%s\\",\\"\%s\\}\)\.{",\%s,\\"\%s\\",\\"\%s\\"%
\\n}\)\.{"},\39\\{swimmer\_name}\?\\{swimmer\_name}:\.{""},\39\\{event\_code},%
\39\\{rows}[\|i].\\{time},\39\\{rows}[\|i].\\{date},\39\\{rows}[\|i].%
\\{standard},\39\\{rows}[\|i].\\{meet});{}$\2\6
\4${}\}{}$\2\6
\&{else}\5
${}\{{}$\1\6
${}\\{printf}(\.{"\%s\ ---\ \%s:\\n"},\39\\{event\_code},\39(\\{opts}\AND\.{OPT%
\_FASTEST})\?\.{"fastest\ time"}:\.{"all\ times\ (fastest\ }\)\.{first)"});{}$\6
...
\M{29}We define a small \PB{\\{Swimmer}} record to hold the person-search
...
a \PB{\\{match\_substr}} (lower-cased) used to identify the correct row, and
a \PB{\\{flag}} bit used to filter output when the user passes a swimmer
keyword
to \.{-o}.
...
\Y\B\4\X29:Main function\X${}\E{}$\6
\&{typedef} \&{struct} ${}\{{}$\1\6
\&{const} \&{char} ${}{*}\\{search\_query}{}$;\C{ Name string to search for  }\6
\&{const} \&{char} ${}{*}\\{match\_substr}{}$;\C{ Lower-case substring to match
}\6
\&{int} \\{flag};\C{ \PB{\.{OPT\_STELLA}}, \PB{\.{OPT\_KALEA}}, etc. }\2\6
${}\}{}$ \&{Swimmer};\6
\&{static} \&{const} \&{Swimmer} \.{SWIMMERS}[\,]${}\K\{\{\.{"Julianna\
Evans"},\39\.{"stella"},\39\.{OPT\_STELLA}\},{}$\C{ Stella Julianna Evans  }\6
${}\{\.{"Benavente"},\39\.{"kalea"},\39\.{OPT\_KALEA}\},{}$\C{ Kalea Rose
Benavente   }\6
${}\{\.{"Ray\ Evans"},\39\.{"kenneth\ ray"},\39\.{OPT\_KENNY}\},{}$\C{ Kenneth
Ray Evans      }\6
${}\{\.{"Santiago\ Evans"},\39\.{"keith\ santiago"},\39\.{OPT\_KEITH}\}{}$\C{
Keith Santiago Evans   }\6
${}\};{}$\6
...
\M{30}\PB{\\{parse\_opts\_str}} tokenises a comma-separated option string (the
argument to \.{-o}) and sets bits in \PB{\\{g\_opts}}.  Unknown tokens are
silently ignored so that future options can be added without breaking
existing invocations.

\Y\B\4\X29:Main function\X${}\mathrel+\E{}$\6
\1\1\&{static} \&{void} \\{parse\_opts\_str}(\&{const} \&{char} ${}{*}\|s)\2%
\2{}$\6
${}\{{}$\C{ Work on a writable copy so strtok can insert NUL bytes. }\1\6
\&{char} ${}{*}\\{buf}\K\\{strdup}(\|s);{}$\7
\&{if} ${}(\R\\{buf}){}$\1\5
\&{return};\2\7
\&{char} ${}{*}\\{tok}\K\\{strtok}(\\{buf},\39\.{","});{}$\7
\&{while} (\\{tok})\5
${}\{{}$\1\6
\&{if} ${}(\\{strcmp}(\\{tok},\39\.{"stella"})\E\T{0}){}$\1\5
${}\\{g\_opts}\MRL{{\OR}{\K}}\.{OPT\_STELLA};{}$\2\6
\&{else} \&{if} ${}(\\{strcmp}(\\{tok},\39\.{"kalea"})\E\T{0}){}$\1\5
${}\\{g\_opts}\MRL{{\OR}{\K}}\.{OPT\_KALEA};{}$\2\6
\&{else} \&{if} ${}(\\{strcmp}(\\{tok},\39\.{"kenny"})\E\T{0}){}$\1\5
${}\\{g\_opts}\MRL{{\OR}{\K}}\.{OPT\_KENNY};{}$\2\6
\&{else} \&{if} ${}(\\{strcmp}(\\{tok},\39\.{"keith"})\E\T{0}){}$\1\5
${}\\{g\_opts}\MRL{{\OR}{\K}}\.{OPT\_KEITH};{}$\2\6
\&{else} \&{if} ${}(\\{strcmp}(\\{tok},\39\.{"fastest"})\E\T{0}){}$\1\5
${}\\{g\_opts}\MRL{{\OR}{\K}}\.{OPT\_FASTEST};{}$\2\6
\&{else} \&{if} ${}(\\{strcmp}(\\{tok},\39\.{"csv"})\E\T{0}){}$\1\5
${}\\{g\_opts}\MRL{{\OR}{\K}}\.{OPT\_CSV};{}$\2\6
${}\\{tok}\K\\{strtok}(\NULL,\39\.{","});{}$\6
\4${}\}{}$\2\6
\\{free}(\\{buf});\6
\4${}\}{}$\2\par
\fi

\M{31}\PB{\\{parse\_events\_str}} tokenises a comma-separated event-code string
(the
argument to \.{-e}) and appends each code to \PB{\\{g\_events}}.  Codes that
would
overflow the \PB{\.{NUM\_EVENTS}}-entry array are silently dropped.  The flag
may
be supplied multiple times; each invocation extends the list.

\Y\B\4\X29:Main function\X${}\mathrel+\E{}$\6
\1\1\&{static} \&{void} \\{parse\_events\_str}(\&{const} \&{char} ${}{*}\|s)\2%
\2{}$\6
${}\{{}$\1\6
\&{char} ${}{*}\\{buf}\K\\{strdup}(\|s);{}$\7
\&{if} ${}(\R\\{buf}){}$\1\5
\&{return};\2\7
\&{char} ${}{*}\\{tok}\K\\{strtok}(\\{buf},\39\.{","});{}$\7
\&{while} ${}(\\{tok}\W\\{g\_nevents}<\.{NUM\_EVENTS}){}$\5
${}\{{}$\C{ Trim leading spaces left by comma-split with spaces around commas.
}\1\6
\&{while} ${}({*}\\{tok}\E\.{'\ '}){}$\1\5
${}\\{tok}\PP;{}$\2\6
${}\\{strncpy}(\\{g\_events}[\\{g\_nevents}],\39\\{tok},\39{}$\&{sizeof} \\{g%
\_events}[\T{0}]${}-\T{1});{}$\6
\\{g\_events}[\\{g\_nevents}][\&{sizeof} \\{g\_events}[\T{0}]${}-\T{1}]\K\.{'%
\\0'};{}$\6
${}\\{g\_nevents}\PP;{}$\6
${}\\{tok}\K\\{strtok}(\NULL,\39\.{","});{}$\6
\4${}\}{}$\2\6
...
\M{32}\PB{\\{print\_usage}} writes the full usage message to standard error.

\Y\B\4\X29:Main function\X${}\mathrel+\E{}$\6
\1\1\&{static} \&{void} \\{print\_usage}(\&{const} \&{char} ${}{*}\\{prog})\2%
\2{}$\6
${}\{{}$\1\6
${}\\{fprintf}(\\{stderr},\39\.{"Usage:\ \%s\ -o\ option}\)\.{[,option...]\ [-e%
\ eve}\)\.{nt[,event...]]\\n"}\.{"\\n"}\.{"\ \ -o\ option,...\ \ \ c}\)%
\.{omma-separated\ outpu}\)\.{t\ options:\\n"}\.{"\ \ \ \ \ \ \ stella\ \ \ \ \
\ }\)\.{restrict\ output\ to\ S}\)\.{tella\ Julianna\ Evans}\)\.{\\n"}\.{"\ \ \
\ \ \ \ kalea\ \ \ \ \ \ \ }\)\.{restrict\ output\ to\ K}\)\.{alea\ Rose\
Benavente\\}\)\.{n"}\.{"\ \ \ \ \ \ \ kenny\ \ \ \ \ \ \ }\)\.{restrict\ output%
\ to\ K}\)\.{enneth\ Ray\ Evans\\n"}\.{"\ \ \ \ \ \ \ keith\ \ \ \ \ \ \ }\)%
\.{restrict\ output\ to\ K}\)\.{eith\ Santiago\ Evans\\}\)\.{n"}\.{"\ \ \ \ \ \
\ fastest\ \ \ \ \ }\)\.{print\ only\ the\ singl}\)\.{e\ fastest\ time\ per\ e}%
\)\.{vent\\n"}\.{"\ \ \ \ \ \ \ csv\ \ \ \ \ \ \ \ \ }\)\.{emit\ CSV\ output\
(hea}\)\.{der\ +\ one\ line\ per\ t}\)\.{ime)\\n"}\.{"\\n"}\.{"\ \ -e\
event,...\ \ \ \ r}\)\.{estrict\ output\ to\ on}\)\.{e\ or\ more\ SCY\ event\ }%
\)\.{codes;\\n"}\.{"\ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ c}\)\.{omma-separated,\
or\ r}\)\.{epeat\ -e\ for\ each\ ev}\)\.{ent.\\n"}\.{"\ \ \ \ \ \ \ Valid\
codes:}\)\.{\ 50\ FR\ SCY,\ 100\ FR\ S}\)\.{CY,\ 200\ FR\ SCY,\ 500\ }\)\.{FR\
SCY,\\n"}\.{"\ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ }\)\.{\ 50\ FL\ SCY,\ 100\
FL\ S}\)\.{CY,\ 50\ BK\ SCY,\ 100\ B}\)\.{K\ SCY,\\n"}\.{"\ \ \ \ \ \ \ \ \ \ \
\ \ \ \ \ \ \ \ }\)\.{\ 50\ BR\ SCY,\ 100\ BR\ S}\)\.{CY,\ 100\ IM\ SCY,\ 200\
}\)\.{IM\ SCY\\n"}\.{"\\n"}\.{"Examples:\\n"}\.{"\ \ \%s\ -o\ stella,fast}\)%
\.{est\\n"}\.{"\ \ \%s\ -o\ kalea,csv\\n}\)\.{"}\.{"\ \ \%s\ -o\ kenny\ -e\ %
\\"}\)\.{100\ FR\ SCY\\"\\n"}\.{"\ \ \%s\ -o\ keith\ -e\ \\"}\)\.{100\ FR\
SCY,50\ FL\ SCY}\)\.{\\"\\n"}\.{"\ \ \%s\ -o\ stella\ -e\ \\}\)\.{"100\ FR\ SCY%
\\"\ -e\ \\"5}\)\.{0\ FL\ SCY\\"\\n"},\39\\{prog},\39\\{prog},\39\\{prog},\39%
\\{prog},\39\\{prog},\39\\{prog});{}$\6
\4${}\}{}$\2\par
\fi

\M{33}We initialise the global \.{libcurl} state, parse the optional
\.{-o} and \.{-e} flags, then for each swimmer (subject to swimmer-selection
bits) resolve her \PB{\\{PersonKey}} and iterate over events.  If one or more
...
${}\{{}$\1\6
\&{int} \\{ch};\7
\&{while} ${}((\\{ch}\K\\{getopt}(\\{argc},\39\\{argv},\39\.{"o:e:"}))\I{-}%
\T{1}){}$\5
${}\{{}$\1\6
\&{switch} (\\{ch})\5
${}\{{}$\1\6
\4\&{case} \.{'o'}:\5
\\{parse\_opts\_str}(\\{optarg});\6
\&{break};\6
\4\&{case} \.{'e'}:\5
\\{parse\_events\_str}(\\{optarg});\6
\&{break};\6
\4\&{default}:\5
\\{print\_usage}(\\{argv}[\T{0}]);\6
\&{return} \T{2};\6
\4${}\}{}$\2\6
\4${}\}{}$\C{ Require at least one option; print usage and exit otherwise. }\2\6
\&{if} ${}(\\{g\_opts}\E\T{0}\W\\{g\_nevents}\E\T{0}){}$\5
${}\{{}$\1\6
\\{print\_usage}(\\{argv}[\T{0}]);\6
\&{return} \T{1};\6
\4${}\}{}$\2\6
\\{curl\_global\_init}(\.{CURL\_GLOBAL\_DEFAULT});\C{ Print CSV header once,
before any swimmer loop. }\6
\&{if} ${}(\\{g\_opts}\AND\.{OPT\_CSV}){}$\1\5
\\{printf}(\.{"\\"Swimmer\\",\\"Event}\)\.{\\",\\"Time\\",\\"Date\\"}\)\.{,%
\\"Standard\\",\\"Meet}\)\.{\\"\\n"});\2\7
\&{int} \\{swimmer\_mask}${}\K\.{OPT\_STELLA}\OR\.{OPT\_KALEA}\OR\.{OPT\_KENNY}%
\OR\.{OPT\_KEITH};{}$\7
\&{for} (\&{int} \|s${}\K\T{0};{}$ ${}\|s<\.{NUM\_SWIMMERS};{}$ ${}\|s\PP){}$\5
${}\{{}$\C{ Skip this swimmer if the -o flag restricts to the other one. }\1\6
\&{if} ${}((\\{g\_opts}\AND\\{swimmer\_mask})\W\R(\\{g\_opts}\AND\.{SWIMMERS}[%
\|s].\\{flag})){}$\1\5
\&{continue};\2\7
...
\&{return} \T{1};\6
\4${}\}{}$\2\6
\&{if} ${}(\R(\\{g\_opts}\AND\.{OPT\_CSV})){}$\1\5
${}\\{printf}(\.{"Swimmer:\ \%s\ \ (Perso}\)\.{nKey:\ \%s)\\n\\n"},\39\\{name}%
\?\\{name}:\.{"(unknown)"},\39\\{key});{}$\2\6
\&{if} ${}(\\{g\_nevents}>\T{0}){}$\5
${}\{{}$\C{ Fetch only the events requested via -e. }\1\6
\&{for} (\&{int} \|i${}\K\T{0};{}$ ${}\|i<\\{g\_nevents};{}$ ${}\|i\PP){}$\1\5
${}\\{fetch\_times}(\\{key},\39\\{g\_events}[\|i],\39\\{name},\39\\{g%
\_opts});{}$\2\6
\4${}\}{}$\2\6
\&{else}\5
${}\{{}$\1\6
\&{for} (\&{int} \|i${}\K\T{0};{}$ ${}\|i<\.{NUM\_EVENTS};{}$ ${}\|i\PP){}$\1\5
${}\\{fetch\_times}(\\{key},\39\.{EVENTS}[\|i],\39\\{name},\39\\{g\_opts});{}$%
\2\6
\4${}\}{}$\2\6
\\{free}(\\{key});\6
\\{free}((\&{void} ${}{*}){}$ \\{name});\6
...
\M{36}{\bf Sisense JAQL API Calls.}
...
\M{38}{\bf POSIX System Calls and Library Functions.}
