...
\Y\B\X6:Includes\X\6
\X7:Global constants\X\6
\X8:Option flags\X\6
\X10:Type definitions\X\6
\X13:HTTP utilities\X\6
\X17:JSON scanner\X\6
\X20:Person lookup\X\6
\X22:Times fetch\X\6
\X26:Main function\X\par
\fi
...
\M{8}The program accepts two optional flags on the command line.
...
\M{10}A \PB{\\{Buffer}} holds a dynamically-grown heap string.  \.{libcurl}
...
\M{11}A \PB{\\{TimeRow}} records one swim result.  \PB{\\{sort\_key}} is a
numeric proxy
...
\N{1}{12}HTTP utilities.

\fi

\M{13}The \.{libcurl} write callback appends each incoming chunk to a
...
\Y\B\4\X13:HTTP utilities\X${}\E{}$\6
...
\M{14}\PB{\\{post\_json}} performs a single HTTP~POST with a JSON body and
returns
the response as a null-terminated heap string, or \PB{$\NULL$} on failure.
The caller is responsible for freeing the returned string.

\Y\B\4\X13:HTTP utilities\X${}\mathrel+\E{}$\6
...
\M{16}Sisense JAQL responses contain a top-level \PB{\.{"values"}} array.  Each
...
\M{17}\PB{\\{scan\_string}} finds the first \PB{\.{"key"}} at or after \PB{${*}%
\\{pos}$}, copies the
...
\M{18}\PB{\\{scan\_long}} is the numeric counterpart: it finds \PB{\.{"key"}},
converts
the digits that follow to a \PB{\&{long}}, stores the result in \PB{${*}%
\\{out}$}, and
advances \PB{${*}\\{pos}$}.

\Y\B\4\X17:JSON scanner\X${}\mathrel+\E{}$\6
...
\N{1}{19}Person lookup. \PB{\\{lookup\_person\_key}} posts a search for \PB{%
...
\M{22}\PB{\\{insertion\_sort}} sorts \PB{\|n} \PB{\&{TimeRow}} records in-place
by \PB{\\{sort\_key}}
...
\M{23}\PB{\\{format\_date}} converts an 8-digit \.{YYYYMMDD} string (such as
\.{20230305}) to \.{YYYY-MM-DD} format in \PB{\\{out}} (which must be at
least 11 bytes).

\Y\B\4\X22:Times fetch\X${}\mathrel+\E{}$\6
...
\M{24}\PB{\\{fetch\_times}} retrieves all SCY times for \PB{\\{person\_key}} in
...
\M{26}We define a small \PB{\\{Swimmer}} record to hold the person-search
...
\M{27}\PB{\\{parse\_opts\_str}} tokenises a comma-separated option string (the
...
\Y\B\4\X26:Main function\X${}\mathrel+\E{}$\6
...
\M{28}\PB{\\{parse\_events\_str}} tokenises a comma-separated event-code string
(the
...
\M{29}\PB{\\{print\_usage}} writes the full usage message to standard error.
...
\M{30}We initialise the global \.{libcurl} state, parse the optional
...
\M{33}{\bf Sisense JAQL API Calls.}
...
\M{35}{\bf POSIX System Calls and Library Functions.}
