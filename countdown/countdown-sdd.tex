...
\code{countdown} is a single-threaded, event-driven program.  A central
loop multiplexes two stimuli---X~events and the passage of time---using one
POSIX \code{select} call, and repaints the window in response to either.
All state lives in one record; the routines are pure functions of that
...
As specified in CD-SRS-001 \S3.6: optional target argument; exit codes
0/1/2; \code{DISPLAY} and \code{TZ} environment variables.
...
typeset program.  The C, \TeX, and PDF outputs are artefacts; the source
of record is \code{countdown.w} (REQ-D-04).
