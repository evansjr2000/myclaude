...
TC-14 & REQ-B-03 & The \code{Makefile} invokes \code{docker-check.sh} (via \code{DOCKER\_CHECK}) as the first command of every container recipe. \\
TC-15 & REQ-B-04 & \code{make -n docs} drives \code{pdftex}/\code{pdflatex} inside the \code{cweb} container for the program listing and all four IEEE/ISO documents; \code{make clean} removes the C, \TeX, executable, and auxiliary artefacts. \\
TC-16 & REQ-B-05 & \code{make -n install} installs the executable into \code{/usr/local/bin} (\code{\$(PREFIX)/bin}) with mode~755. \\
\bottomrule
\end{longtable}
