...
CD-STP-001 Rev.\,1.1, \textit{Software Test Plan: countdown}.
...
TC-14 & REQ-F-08 & The source recognises the \code{-gui} option and provides a terminal runner; with no \code{-gui} and no \code{DISPLAY}, a near-future target counts down in the terminal, announces arrival, and exits~0 (needs no X~server). \\
TC-15 & REQ-F-09 & Terminal mode rewrites the remaining time on a single line in place (leading carriage return). \\
TC-16 & REQ-B-03 & The \code{Makefile} invokes \code{docker-check.sh} (via \code{DOCKER\_CHECK}) as the first command of every container recipe. \\
TC-17 & REQ-B-04 & \code{make -n docs} drives \code{pdftex}/\code{pdflatex} inside the \code{cweb} container for the program listing and all four IEEE/ISO documents; \code{make clean} removes the C, \TeX, executable, and auxiliary artefacts. \\
TC-18 & REQ-B-05 & \code{make -n install} installs the executable into \code{/usr/local/bin} (\code{\$(PREFIX)/bin}) with mode~755. \\
\bottomrule
\end{longtable}
...
\midrule
\endhead
MC-01 & REQ-G-01, REQ-G-02 & Launch \code{countdown -gui}; a window opens showing target, remaining time, status, and hint. \\
MC-02 & REQ-F-01, REQ-T-01 & With \code{-gui} and no target, the window counts down toward the default target, the seconds advancing once per second. \\
...
MC-06 & REQ-F-07 & Launch with \code{-gui} and a target a few seconds ahead; on arrival the status shows the arrival message and the figure reads \code{0 d 00:00:00}. \\
MC-07 & REQ-F-08, REQ-F-09 & Launch \code{countdown} with no \code{-gui} (optionally over a display-less shell); the countdown runs in the terminal, rewriting one line in place once per second, with no window. \\
\bottomrule
\end{longtable}
...
REQ-F-08 & TC-14, MC-07 & Automated (static + dynamic) + Manual \\
REQ-F-09 & TC-15, MC-07 & Automated (static) + Manual \\
