...
\code{countdown} is a single-window X~Window System application that
displays the time remaining until a fixed future instant and updates that
display live.  The SRS covers the program tangled from
\code{countdown.w}.  It does not cover the X~server, the operating
system's time-zone database, or the CWEB tools themselves, except to state
the program's reliance on them.
...
\midrule
\endhead
\rrow{REQ-G-01}{The program shall present its output in a graphical window
  on the X~Window System using the Xlib client interface.}
...
\textbf{Command-line interface.}  \code{countdown [TARGET]}, where the
optional \code{TARGET} is a string of the form \code{"YYYY-MM-DD HH:MM:SS"}
(REQ-F-03).  Exit status is \code{0} on normal termination, \code{1} on
inability to open the display, and \code{2} on a malformed target
(REQ-F-04).
...
REQ-B-01--REQ-B-02 & \S6.2 operational environment \\
\bottomrule
\end{longtable}
