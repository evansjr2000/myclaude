...
% Software Test Plan for the AWEB toolchain (atangle/aweave) release 2.1.
...
\hypersetup{
  pdftitle  = {AWEB-STP-2.1: Software Test Plan},
...
\newcommand{\stpid}{AWEB-STP-2.1}
\newcommand{\sut}{\textsf{atangle}/\textsf{aweave}}

\title{Software Test Plan\\\large AWEB Toolchain (\textsf{atangle} 2.1 / \textsf{aweave} 2.1)}
...
This is the Software Test Plan (STP) for release 2.1 of the AWEB
...
\end{quote}
Release~2.1 adds a \texttt{-v} / \texttt{--version} command-line option
to both programs.  When the option is given, the program prints its
banner and exits successfully without opening any input or output
files.  The container is now tagged \texttt{evansjr/awebpg:2.1}.

Release~2.1 also removes the PostgreSQL stack that was bundled in
release~2.0 (requirement~10).  The image now contains only the
toolchain (\texttt{atangle}, \texttt{aweave}) and supporting utilities
(\texttt{make}, \texttt{file}, \texttt{emacs}); the corresponding
PostgreSQL test case (TC--012) is withdrawn.

This STP exercises the unchanged behaviour (regression), the Ada~95
keyword classification, the new version-flag behaviour, and the
surrounding container artefacts (\texttt{Dockerfile},
\texttt{awebpg.sh}, \texttt{docker-entrypoint.sh}).
...
\midrule
TI--1 & \texttt{atangle} executable   & 2.1 & \texttt{atangle/atangle.adb} (+ tangled units)\\
TI--2 & \texttt{aweave}  executable   & 2.1 & \texttt{aweave/aweave.adb}  (+ tangled units)\\
TI--3 & \texttt{Dockerfile}           & 2.1 & \texttt{./Dockerfile}\\
TI--4 & \texttt{awebpg.sh} driver     & 2.1 & \texttt{./awebpg.sh}\\
TI--5 & \texttt{docker-entrypoint.sh} & 2.1 & \texttt{./docker-entrypoint.sh}\\
\bottomrule
\end{tabular}
\end{center}
Each item is delivered as part of a single OCI image tagged
\texttt{evansjr/awebpg:2.1}.
...
NF--1 & Ada~2005/Ada~2012 keywords (\texttt{interface},
      ...
      & Out of scope for release~2.1; only Ada~95 was contracted.\\
...
Contains ``This is ATANGLE, Version 2.1 (Ada 95)''.\\

TC--004 & aweave banner & F--2 &
\texttt{aweave ada83.aweb} ; first stdout line. &
Contains ``This is AWEAVE, Version 2.1 (Ada 95 keyword support)''.\\
...
\texttt{docker build -t evansjr/awebpg:2.1 .} &
Build succeeds; image carries label
\texttt{org.opencontainers.image.version=2.1}.\\
...
files are produced. &
Banner contains
``This is ATANGLE, Version 2.1 (Ada 95)''; no artefacts.\\
...
files are produced. &
Banner contains
``This is AWEAVE, Version 2.1 (Ada 95 keyword support)'';
no artefacts.\\
\bottomrule
\end{longtable}
}
